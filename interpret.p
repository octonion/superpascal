...
begin
  ...
  //writeln('exit load, stackbottom = ',i);
end;
...
procedure activate(
  ...
  integer);
begin
  //writeln('enter activate');
  svalue := svalue + 3;
  st[svalue - 2] := pvalue;
  st[svalue - 1] := bvalue;
  st[svalue] := ready;
  ready := svalue;
  //writeln('exit activate');
end;

procedure select(
  lineno: integer);
begin
  //writeln('enter select');
  //writeln('ready =',ready);
  if ready = 0 then
...
var i: integer;
begin
  //writeln('enter popstring');
  // writeln('s = ',s);
  s := s - maxstring;
  for i := 1 to maxstring do
     value[i] := chr(st[s + i]);
  // writeln('value = ',value);
  //writeln('exit popstring');
end;

begin
  //writeln('enter run');
  //writeln(outfile,'enter run');
  load(codefile);
  p := minaddr;
  running := true;
  while running do
    if (p>stackbottom) then
       running := false
    else
    case st[p] of
...
begin
  ...
  // writeln('enter readtime');
  t := fptime(t);
  // writeln('time = ',t);
  // writeln('exit readtime');
end;
...
begin
  writeln('openoutput');
  if copy(outname,1,phraselength(outname)) = 'screen' then
    begin
      writeln(output);
      runtime(codefile,
        inpfile, output);
    ...
    begin
      { nonstandard rewrite }
      assign(outfile, copy(outname,1,phraselength(outname)));
      writeln('rewrite(outfile) 1');
      rewrite(outfile);
      writeln('rewrite(outfile) 2');
      runtime(codefile,
        inpfile, outfile)
    end
end;
...
begin
  writeln('openinput');
  writeln(inpname,outname);
  if copy(inpname,1,phraselength(inpname)) = 'keyboard' then
    openoutput(codefile,
      input, outname)
  else
    begin
      { nonstandard reset }
      assign(inpfile, copy(inpname,1,phraselength(inpname)));
      reset(inpfile);
      openoutput(codefile,
        inpfile, outname)
    end
...
begin
  ...
  if select then
    begin
      ...
      readphrase(outname);
      { nonstandard reset }
      assign(codefile, copy(codename,1,phraselength(codename)));
      reset(codefile);
      openinput(codefile,
        inpname, outname)
    end
